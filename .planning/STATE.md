...
**Active blocker (deferred, not stuck; narrower than before):** After the previous entries below were written, the user asked whether a compile could actually be attempted. It could, partially: a portable JDK 21 (aarch64) + Android SDK (cmdline-tools, platform-36, build-tools) was set up entirely in user space (`~/.cache/android-toolchain`, no sudo, no system changes) and used to (1) confirm — via a real `./gradlew` invocation — that a genuine, real gap existed and was fixed: **this project had no Kotlin Gradle plugin configured at all**, so none of Phase 8/9's six `.kt` files would ever have reached the Kotlin compiler; and (2) after fixing that, compile all four Kotlin files plus the modified `MainActivity.java` directly against a hand-assembled classpath (extracted from Gradle's own dependency resolution) — **clean compile, one pre-anticipated deprecation warning, zero errors**. `AndroidManifest.xml`'s merge also already succeeded in the same attempt (before the failure below), confirming the new permissions/`<service>` block are valid.

What's still not possible here: producing a real, installable APK. AGP's `aapt2` (resource compiler) Maven artifact only ships x86_64 Linux binaries, and this host is aarch64 with no cross-arch emulation available without root (`sudo` requires an interactive password not available in this session). This is a host-architecture limitation, not a code problem — it would not reproduce on the user's own Windows machine (`gradlew.bat`), which is a real x86_64/ARM64-native Windows toolchain, not this constrained scenario.

Still unverified regardless of the above: the manifest/foreground-service-type pairing's `SecurityException` risk (needs an actual app install), wake-lock timer accuracy under real Doze/idle conditions, whether Skip/+15s/Finish actually survive a real killed process, whether the resume-drain path fires correctly on a frozen (not killed) WebView, and whether the ducked audio actually ducks real music — all genuinely device-only checks. The user has explicitly chosen to have all the implementation work (and now this compile verification) done now, and run the remaining on-device checks on their own time.
