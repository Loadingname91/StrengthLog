...
v1.2 "Reliable Alerts" milestone: all three phases are implemented. Phase 7 is verified/shipped. Phase 8 (Ongoing Workout Notification) and Phase 9 (Notification Polish) are both implemented (`08-01`/`08-02`/`09-01`/`09-02` PLAN.md and SUMMARY.md files) but not yet verified — this session has no Android SDK, so the on-device steps that decide whether NOTIF-09..17 actually work remain the user's next step on a real device. The user explicitly chose to have both phases implemented in one session rather than pausing after Phase 8 to verify first.
...
| Rest-done audio ducking via `AudioManager.requestAudioFocus` + `MediaPlayer` (USAGE_ALARM), not the notification channel's default sound | Channel-driven sound doesn't request audio focus and so can't duck concurrent music — the whole point of NOTIF-16; a one-line pre-API-26 fallback covers the declared minSdk-24 floor since no AndroidX compat class exists for audio focus below API 26 | Shipped (pending device verification) — Phase 9 |
| Notification's "Finish" tap routes through `WorkoutService` (enqueue + `startActivity`), reusing Phase 8's pending-action pipeline, rather than a second delivery mechanism | 08-CONTEXT.md's original sketch suggested a separate `MainActivity.onNewIntent` path; working through what already exists made a second mechanism redundant — `ACTION_FINISH_TAPPED` is just a third pending-action type | Shipped (pending device verification) — Phase 9 |
| `FINISH_TAPPED` sets a `SET_FINISH_REQUESTED` flag rather than dispatching `FINISH_WORKOUT` from the notification effect layer | Only `ActiveWorkout.jsx` knows `allSetsLogged`; the effect layer stays workout-content-agnostic and lets the screen's own existing `finish()` decide confirm-sheet vs. direct finish | Shipped (pending device verification) — Phase 9 |
| Effect E drains the pending-action queue on Capacitor's `resume` event, not just on mount | Phase 8's mount-only drain covers a killed-and-restarted process but misses the app merely being backgrounded with a frozen (not killed) WebView — a real gap in NOTIF-14's "correctly replayed... on resume" language | Shipped (pending device verification) — Phase 9 |
| Battery-manager guidance is plain Settings text, no `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` call | Explicitly out of scope per 09-CONTEXT.md D-04 — documentation as the fix, not a workaround | Shipped — Phase 9 |
| Phase 9 planned and executed immediately after Phase 8 in the same session, without waiting for Phase 8's device verification | User's explicit instruction: "work on it, I'll do device verification later" | Shipped (pending device verification) — Phase 9 |
