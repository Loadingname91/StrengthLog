...
- **Tech stack**: React 19, Vite, Tailwind v4, react-router v7, Capacitor 8 (Android + iOS) — stay within this stack, no new frameworks
- **Data**: fully local via `localStorage`, offline-first — no backend/sync work in this milestone
- **Platform**: Android via Capacitor; native build runs from `android/` on Windows (`gradlew.bat`), web assets rebuilt via `npm run build && npx cap sync android` before each native install. iOS platform added via `npx cap add ios` (`ios/` directory, Swift Package Manager for plugins — no CocoaPods/Podfile); CI (`.github/workflows/ios-simulator-build.yml`) builds Simulator-only (`CODE_SIGNING_ALLOWED=NO`, no Apple Developer account) since there's no paid Apple Developer enrollment yet — a real-device/TestFlight build needs one plus a signing cert/provisioning profile as GitHub secrets
...
- Swift (iOS native shell) - `ios/` directory generated by Capacitor for the native iOS build; plugins resolve via Swift Package Manager (`ios/App/CapApp-SPM/Package.swift`), no CocoaPods
...
- Capacitor 8.5.1 (`@capacitor/core`, `@capacitor/android`, `@capacitor/ios`, `@capacitor/app`, `@capacitor/cli`) - Native app shell/bridge for packaging the web app as an Android and iOS app; config at `capacitor.config.json` (appId `com.fitlog.app`, appName `FitLog`, webDir `dist`)
...
- `@capacitor/core` / `@capacitor/android` / `@capacitor/ios` / `@capacitor/app` - Enables the web app to run as a native Android/iOS app and handle native events (e.g., Android back button, `src/App.jsx`)
...
- `ios/` - generated native iOS project (Xcode/Swift Package Manager-based), created/managed by Capacitor CLI
...
- For native iOS builds: a Mac with Xcode (`ios/App/App.xcodeproj`, scheme `App`); plugins resolve via Swift Package Manager automatically during the Xcode build, no separate `pod install` step
...
- iOS: currently Simulator-only builds (CI: `.github/workflows/ios-simulator-build.yml`, unsigned, `generic/platform=iOS Simulator`) — no Apple Developer Program enrollment yet, so nothing installable on a real device or distributable via TestFlight exists. Running on a personal iPhone today requires a Mac + Xcode + a free Apple ID (7-day-expiring signing); a real CI-built `.ipa` needs a paid ($99/yr) Apple Developer account plus a signing certificate/provisioning profile added as GitHub secrets
