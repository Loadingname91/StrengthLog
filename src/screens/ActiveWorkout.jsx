...
function ExpandedExercise({ unit, index, restUntil, restExerciseIndex, restSetIndex, restRemaining, onSwap, now }) {
  ...
  // Groups a superset's flat sets array into rounds by roundIndex (falling
  // back to positional math for older shapes without it) — a round can hold
  // fewer than unitExerciseIds.length sets for an uneven superset, so
  // grouping by position alone would misalign rows.
  const supersetRounds = []
  if (isSuperset) {
    unit.sets.forEach((set, si) => {
    ...
    <div className="rounded-[20px] border p-4" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="mt-0.5 text-xs" style={{ color: 'var(--muted)' }}>
            {isSuperset && unit.exercises?.length
              ? unit.exercises.map((e) => `${exerciseById(e.exerciseId, exercises)?.name || e.exerciseId}: ${e.target || blockTarget(e)}`).join(' • ')
              : `Target ${unit.target}`}
          </div>
        </div>
          ...
          return (
            <div key={si}>
              <SetRow
                ...
                set={set}
                ghost={ghostFor(si)}
                targetWeight={set.targetWeight ?? unit.targetWeight}
      ...
      {isSuperset &&
        supersetRounds.map((round, rIdx) => {
          ...
          const roundRestSeconds = unit.restAfter[lastItem.si]
          const roundRestState = !lastItem.set.done ? 'upcoming'
            : (restUntil && restExerciseIndex === index && restSetIndex === lastItem.si) ? 'active'
              : 'passed'
          return (
            <div key={`round-${rIdx}`} className="mt-2.5 rounded-xl p-2.5" style={{ background: 'var(--surface-alt)' }}>
              ...
              {round.map(({ set, si }) => {
                const exId = set.exerciseId || unitExerciseIds[set.exerciseIndex] || unit.exerciseId
                return (
                  <div key={si}>
